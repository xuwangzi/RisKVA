...
if __name__ == "__main__":
    ...
    finetuned_model_path = "models/finetuned_models/RisKVA/RisKVA-Subunit-7B_3_epoch"
    ...
    while True:
        # 输入图片数量和图片路径
        img_count = int(input("请输入图片数量："))
        image_paths = []
        for i in range(img_count):
            img_path = input(f"请输入第{i+1}张图片的路径：")
            image_paths.append(img_path)

        # 计时开始
        start_time = time.time()

        result = finetuned_model.inference_with_images(image_paths) # 字典格式
        ...
        print(f"⏱️推理耗时：{elapsed_time:.2f} 秒")