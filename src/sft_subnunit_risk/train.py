...
if __name__ == "__main__":
    ...
    def prepare_dataset(dataset: DatasetDict, dataset_path: str, dataset_train_split: str, templates: Optional[Dict[str, Any]] = None) -> DatasetDict:
        ...
        # 对数据集进行转换
        def transform_dataset(dataset_split):
            transformed_data = {
                'messages': [],
                'images': []
            }
        ...
        # 转换训练集
        train_data = transform_dataset(dataset[dataset_train_split])
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            print(f"example: {example}") # ♻️ print()导致PIL图像对象被强制加载到内存，从而增加内存引用和GPU显存占用
            # print(f"type(example): {type(example)}")
            # print(f"example['images']: {example['images']}")
            # print(f"type(example['images']): {type(example['images'])}")
            # print(f"example['messages']: {example['messages']}")
            # print(f"type(example['messages']): {type(example['messages'])}")
        ...
        # 🔍 ♻️ 调试 - 保留关键显存监控
        # 显示GPU占用情况 (格式: GPU_ID, 名称, 利用率%, 已用显存MB, 总显存MB)
